...
const MAX_TOKENS = 18000; // 매크로 요약 + 핵심 테마 10(기존) + 보강 상세 + 30개 헤드라인(한경·매경·Reuters) + 컨센서스 5 — 전 섹션 테이블화 대비
...
moneytree-web의 기존 SSOT 테마 인덱스에만 매핑합니다(신규 테마 제안 없음).
...
4. ⚠️ 신규 테마 제안 금지. 오직 아래 SSOT 인덱스에 이미 존재하는 T_xxx 테마만 사용한다. 오늘 뉴스와 가장 관련 깊은 기존 테마 정확히 10개를 선정해 제시한다.
...
## 1. 오늘의 핵심 테마 10 (기존 SSOT 테마만)
오늘 뉴스와 가장 관련 깊은 **기존 인덱스 테마 정확히 10개**. 신규 테마 제안 금지 — 반드시 SSOT의 실제 T_xxx만.
...
## 2. 기존 테마 보강 상세
섹션 1의 10개 테마 각각에 대해, 오늘 뉴스로 편입/강화할 자산·관계를 정리.

| 테마 ID | 테마명 | 추가/주목 자산 (ticker/exchange) | 관계 (THEMED_AS·OPERATES·SUPPLIES 등) | 근거 + 원문 URL |
|---|---|---|---|---|
| T_xxx | ... | T1/EX1 | THEMED_AS | "..." [원문](URL) |

## 3. 소스별 핵심 요약 (모두 테이블)

### 3-1. Bloomberg Technology
...
- 섹션 1은 반드시 SSOT 인덱스의 기존 T_xxx 10개. 인덱스에 없는 테마 생성·제안 금지.
