...
// 52주 최고가 대비 <-50% — 전체 ASSET 중 52주 최고가에서 -50% 넘게 이탈(hg ≤ -50%)한 종목만 이동평균선 브리핑.
// 데이터: kang0302/import_MT/main/data/ma_brief/assets.json (전체 테마 ASSET, 매일 GitHub Actions 갱신, items=dict)
...
const HG_THRESHOLD = -50; // 52주 최고가 대비 -50% 넘게 이탈
...
export default function Low52wPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const r = await fetch(`${ASSETS_URL}?_cb=${Date.now()}`, { cache: "no-store" });
        if (!r.ok) { if (!cancelled) setState(r.status === 404 ? "empty" : "error"); return; }
        const j = (await r.json()) as Payload;
        if (!cancelled) { setData(j); setState(j?.items && Object.keys(j.items).length ? "ok" : "empty"); }
      } catch { if (!cancelled) setState("error"); }
    })();
    return () => { cancelled = true; };
  }, [nonce]);

  // 전체 자산 → 52주 최고가 대비 -50% 넘게 이탈(hg ≤ -50%)만
  const universe = useMemo(() => {
    const all = data ? Object.values(data.items) : [];
    return all.filter((r) => r.hg != null && r.hg <= HG_THRESHOLD);
  }, [data]);
      ...
      <div className="mx-auto w-full max-w-[1600px] px-3 py-5">
        ...
        <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-lg font-semibold text-white/90">📉 52주 최고가 대비 &lt;-50% (이동평균선 브리핑)</h1>
          </div>
        ...
        {state === "ok" && data && (
          <>
            <div className="mb-3 rounded-lg border border-rose-400/20 bg-rose-500/[0.06] px-3 py-2 text-xs text-rose-100/80">
              전체 자산 <b className="text-white/90">{data.count}</b>종 중 <b className="text-rose-200">52주 최고가 대비 -50% 넘게 이탈</b>(대낙폭) <b className="text-white/90">{universe.length}</b>종만 이평선 브리핑합니다.
            </div>
            ...
            <p className="mt-3 text-[11px] leading-relaxed text-white/40">
              전체 ASSET 중 <b className="text-white/60">52주 최고종가 대비 -50% 넘게 이탈</b>(52주高比 ≤ -50%)한 종목만 표시. ▲(적) 상회·상승 / ▼(청) 하회·하락. 52주高比=1년 최고종가 대비 격차. 헤더 클릭 시 정렬(재클릭=방향전환). 행에 마우스를 올리면 해석이 뜹니다. 테마번호 클릭 시 그래프 페이지로 이동.
            </p>
          </>
        )}
      </div>
