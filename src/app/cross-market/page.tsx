...
type Lead = { hitRate: number; corr: number; n: number; lastUsRet: number | null; lastUsDate: string; pending: boolean };
    ...
    <main className="min-h-screen w-full bg-black text-white">
      <div className="mx-auto w-full max-w-[1200px] px-4 py-6">
        ...
        <p className="mb-5 text-[12.5px] leading-relaxed text-white/55">
          ...
          그리고 <b className="text-white/80">어젯밤 미국 흐름이 오늘 한국</b>으로 이어지는지를 봅니다. <span className="text-white/40">(예측이 아닌 탐지·경보)</span>
        </p>
        ...
        {state === "ok" && data && (
          <>
            ...
            <section className="mb-7">
              <h2 className="mb-1 text-sm font-semibold text-amber-200/85">🌙 US→KR 오버나잇 리드 <span className="text-white/35">{leads.length}</span></h2>
              <p className="mb-2.5 text-[11px] text-white/45">어젯밤 <b className="text-white/60">미국</b> 세션이 오늘 <b className="text-white/60">한국</b> 세션으로 이어진 <b className="text-white/60">방향 적중률</b>이 높은 테마. 어젯밤 미국 움직임으로 오늘 한국 방향을 가늠합니다.</p>
                  ...
                  return (
                    <a key={t.id} href={`/graph/${t.id}`} target="_blank" rel="noreferrer"
                      ...
                      <div className="flex items-center justify-between gap-2">
                        <div className="text-[11px] text-white/50">어젯밤 미국<br /><span className="text-[16px] font-bold tabular-nums" style={{ color: retColor(L.lastUsRet) }}>{pct(L.lastUsRet, 2)}</span></div>
                        <div className="text-center text-[11px] text-white/40">→</div>
                        <div className="text-right text-[11px] text-white/50">오늘 한국 예상<br /><span className="text-[16px] font-bold" style={{ color: up ? "#f87171" : "#60a5fa" }}>{up ? "▲ 상승" : "▼ 하락"}</span></div>
                      </div>
