...
// 52주 신고가 근접 — 전체 ASSET 중 52주 최고가 대비 -10% 이내(hg ≥ -10%) 종목만 이동평균선 브리핑.
      ...
      <div className="mx-auto w-full max-w-[1600px] px-3 py-5">
        <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-lg font-semibold text-white/90">🚀 52주 신고가 근접 (이동평균선 브리핑)</h1>
          </div>
        ...
        {state === "ok" && data && (
          <>
            <div className="mb-3 rounded-lg border border-emerald-400/20 bg-emerald-500/[0.06] px-3 py-2 text-xs text-emerald-100/80">
              전체 자산 <b className="text-white/90">{data.count}</b>종 중 <b className="text-emerald-200">52주 최고가 대비 -10% 이내</b>(신고가 근접) <b className="text-white/90">{universe.length}</b>종만 이평선 브리핑합니다.
            </div>
            ...
            <p className="mt-3 text-[11px] leading-relaxed text-white/40">
              전체 ASSET 중 <b className="text-white/60">52주 최고종가 대비 -10% 이내</b>(52주高比 ≥ -10%) 종목만 표시. ▲(적) 상회·상승 / ▼(청) 하회·하락. 52주高比=1년 최고종가 대비 격차. 헤더 클릭 시 정렬(재클릭=방향전환). 행에 마우스를 올리면 해석이 뜹니다.
            </p>
          </>
        )}
      </div>
